        ...
        <el-row class="typeItem line">
          ...
          <el-col :span="20">
            <ul>
              <li
                @click="typeChange(0)"
                v-bind:class="ids.length === 0 ? activeclass : ''"
              >
                全部
              </li>
              ...
              <li
                ...
                v-bind:class="ids.indexOf(item.id) > -1 ? activeclass : ''"
          ...
          <el-col :span="20">
            <ul>
              <li
                @click="priceChange(0)"
                v-bind:class="
                  Object.keys(this.priceInterval).length === 0
                    ? activeclass
                    : ''
                "
              >
              ...
              <li
                ...
                v-bind:class="item.min === priceInterval.min ? activeclass : ''"
                v-for="item in typePrice"
                :key="item.min"
              >
                {{ item.min }}-{{ item.max }}
              </li>
              <li
                @click="priceChange(-1)"
                v-bind:class="priceInterval.max > 1000 ? activeclass : ''"
              >
                1000以上
              </li>
            </ul>
        ...
        <el-row class="typeItem line">
          ...
          <el-col :span="20">
            <ul>
              <li
                @click="timeChange(0)"
                v-bind:class="this.timeInterval === 0 ? activeclass : ''"
              >
                全部
              </li>
              <li
                @click="timeChange(item.number)"
                v-bind:class="item.number === timeInterval ? activeclass : ''"
            ...
            <ul>
              <li v-for="item in typeProducts" :key="item.id">
                <div class="containerProduct">
                  ...
                  <p class="productName" @click="gotoDetails(item.id)">
                    {{ item.name }}
                  </p>
                  <div class="userPlay">
                    ...
                    <span>
                      ...
                      <button v-else @click="showUserInfo(item.uid)">
                        联系卖家
                      </button>
                    </span>
                  </div>
...
export default {
  ...
  data() {
    return {
      ...
      typeTime: [
        { number: 1, text: "刚刚发布" },
        { number: 7, text: "7天以内" },
        { number: 30, text: "30天以内" },
        { number: 60, text: "60天以内" },
        { number: 90, text: "90天以内" },
        { number: 365, text: "一年以内" }
      ], //时间区间数组
      ids: [], //二级分类id列表
  ...
  methods: {
    ...
    loadProducts() {
      //加载所属二级分类商品
      this.$axios
        .post("/product/selectScProductById", {
    ...
    addCart(data) {
      ...
      if (this.$store.state.userid !== 0) {
        ...
        this.$axios
          .post("/cart/selectCartProductById", {
            pid: data.id,
    ...
    showUserInfo(id) {
      this.$axios
        ...
        .then(res => {
          this.$alert(
            '<div><span  style="font-weight:600;margin-left:30px;">用户名：</span>' +
              res.data.username +
              "</div>" +
              '<div><span  style="font-weight:600;margin-left:30px;">联系电话：</span>' +
              res.data.phonenumber +
              "</div>" +
              '<div><span  style="font-weight:600;margin-left:30px;">E-mail：</span>' +
              res.data.mail +
              "</div>",
            "卖家联系方式",
            {
